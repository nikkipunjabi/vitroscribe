...
class MeetingDetector: ObservableObject {
    ...
    private func checkForActiveMeetings() {
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            if !exactMeetingMatch && isBrowserOpen {
                ...
                for browser in browsers {
                    if runningAppNames.contains(browser) {
                        if let urls = self.getActiveTabURLs(for: browser) {
                            for url in urls {
                                if self.isRealMeetingURL(url) {
            ...
            if !exactMeetingMatch && !self.isScreenRecordingAuthorized && isBrowserOpen {
                let now = Date()
            ...
            if !meetingFound && self.consecutiveHits == 0 {
                if diagnosticLog.contains("Chrome") && !self.isScreenRecordingAuthorized {
